...
const travelUi = readFileSync(join(projectPath, 'src', 'ui', 'TravelPlannerController.ts'), 'utf8');
...
assert(!/StoryCampaign|storyCampaign|storyBeat|storyFlow/.test(main), 'Legacy Story Flow implementation remains in the editor application.');
...
for (const feature of ['renderNPCList', 'regenerateNpcRoster', 'travelPlanner']) {
  assert(main.includes(feature), `NPC/travel integration missing: ${feature}`);
}
for (const feature of ['handleMapPoint', 'renderAlternatives', 'alternativesValue', 'customLocations']) {
  assert(travelUi.includes(feature), `Travel planner controller feature missing: ${feature}`);
}
...
execFileSync(process.execPath, [join(outputPath, 'tests', 'NpcTravelNetworkTest.js')], { cwd: projectPath, stdio: 'inherit' });
