...
const shell = read('src', 'ui', 'CampaignStudioShell.ts');
const css = read('src', 'ui', 'campaign-studio-shell.css');
...
const main = withoutComments(read('src', 'EditorApplication.ts'));
...
const storyCommunity = JSON.parse(execFileSync(process.execPath, [join(outputPath, 'tests', 'StoryRemovalCommunityTest.js')], { cwd: projectPath, encoding: 'utf8' }));
const result = {
  ...
  storyPointRemoval: storyCommunity.storyPointsAfterRemoval === storyCommunity.baselineStoryPoints - 1,
