-- PAYAW room/player-portal API repair.
-- Recreates the RPCs used by the current frontend and explicitly asks PostgREST
-- to reload its schema cache. Safe to apply after migrations 001-006.

create extension if not exists pgcrypto with schema extensions;
create schema if not exists private;
revoke all on schema private from public, anon, authenticated;

-- Room creation is intentionally idempotent. Repeated clicks or a stale local
-- room cache reconnect to the same room for the same GM + source campaign.
create or replace function public.create_campaign_room(
  p_name text,
...
begin
  ...
  v_source_campaign_id := left(
    coalesce(nullif(trim(p_source_campaign_id), ''), extensions.gen_random_uuid()::text),
    160
  );
  v_name := left(coalesce(nullif(trim(p_name), ''), 'PAYAW Campaign'), 160);

  insert into public.campaign_rooms(owner_user_id, source_campaign_id, name)
  values (v_user, v_source_campaign_id, v_name)
  on conflict (owner_user_id, source_campaign_id) do update set
    name = excluded.name,
    updated_at = now()
  ...
  values (
    ...
    v_user,
    'owner-gm',
    left(coalesce(nullif(auth.jwt()->>'email', ''), 'Game Master'), 80)
  )
  ...
  insert into public.campaign_authority(campaign_id, updated_by)
  ...
  insert into public.campaign_audit_log(
    ...
    v_room, v_user, 'room.create-or-link', 'campaign', v_room::text
  );
...
revoke all on function public.create_campaign_room(text, text) from public, anon;
...
-- Ask Supabase PostgREST to see the recreated functions immediately instead of
-- waiting for its internal schema cache refresh interval.
