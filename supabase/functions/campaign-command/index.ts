...
function applyCommand(projectionValue: unknown, kind: string, payloadValue: unknown, now: string): JsonRecord {
  ...
  if (kind === 'journal.create') {
    ...
    const visibility = ['private', 'party', 'gm'].includes(text(payload.visibility, 12)) ? text(payload.visibility, 12) : 'private';
    const values = Array.from({ length: dice.count }, () => secureRoll(dice.sides));
    const roll = { id: id('roll'), notation: dice.notation, values, modifier: dice.modifier, total: values.reduce((sum, value) => sum + value, 0) + dice.modifier, visibility, rolledAt: now };
...
function partyDelta(projection: JsonRecord, kind: string, payload: JsonRecord): JsonRecord | null {
  ...
  if (kind === 'dice.roll' && payload.visibility === 'party') {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const partyVisible = body.kind === 'message.send' && body.payload.privateToGm !== true
      || body.kind === 'dice.roll' && body.payload.visibility === 'party'
      || body.kind === 'map.ping'
    ...
    const gmOnly = body.kind === 'message.send' && body.payload.privateToGm === true
      || body.kind === 'dice.roll' && body.payload.visibility === 'gm';
    await service.from('campaign_events').insert({
      ...
      revision: nextRevision, safe_payload: { commandId: command.id, sourcePlayerId: slot.source_player_id },
    });
    await service.from('campaign_commands').update({ status: 'applied', result: { revision: nextRevision }, resolved_at: now }).eq('id', command.id);
    return json(200, { command: { ...command, status: 'applied', result: { revision: nextRevision } }, projection: updated.projection });
  } catch (error) {
