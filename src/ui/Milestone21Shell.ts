...
export function installMilestone21Shell(): void {
  ...
  const activePanels: Record<Workspace, PanelKey> = { editor: 'generate', dm: 'dashboard' };
  ...
  const editorGroups: Record<string, readonly HTMLElement[]> = {
    ...
    anchors: [
      ...
      editorWorkspace.querySelector<HTMLElement>('.anchor-editor'),
    ].filter((value): value is HTMLElement => value !== null),
