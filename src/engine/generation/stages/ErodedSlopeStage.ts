...
export class ErodedSlopeStage implements GenerationStage {
  public readonly id = 'eroded-slope';
  public run(context: Parameters<GenerationStage['run']>[0]): void {
    ...
    for (let index = 0; index < world.tiles.length; index += 1) {
      ...
      if (tile === undefined || slope === undefined) throw new Error('Eroded slope stage encountered an invalid tile.');
      tile.slope = slope;
    }
  }
}
